...
# from SNMF import PlotDecomposition as sp
...
def normalize_samples(bootstrapGenomes,totalMutations,norm="100X", normalization_cutoff=10000000):
    ...
    if norm=="gmm":
        ...
        norm_genome = bootstrapGenomes[:, list(indices)] / totalMutations.to_numpy()[list(indices)][:, np.newaxis].T * normalization_cutoff
        ...
        norm_genome = bootstrapGenomes[:, list(indices)] / totalMutations.to_numpy()[list(indices)][:, np.newaxis].T * (rows * 100)
        bootstrapGenomes[:,list(indices)] = norm_genome
        bootstrapGenomes = pd.DataFrame(bootstrapGenomes)
    elif norm == "log2":
...
def decipher_signatures(execution_parameters, genomes=[0], Y=0, lambda_c= 1e-40, lambda_p= 0.5, lr=0.002, i=1, refit = False, W = 0, totalIterations=1, cpu=-1, mut_context="96", noise_rep_pair=None, path = None):
    ...
    max_loss = np.amax(Ltot)
    threshold_percent = int(round((threshold - 1) * 100))  # e.g., 1.2 -> 20%

    print(Ltot)
    print('Min Loss:', min_loss)
    print('Max Loss:', max_loss)
    print(f'Threshold ({threshold_percent}%) :', threshold * min_loss)
    print('Ratio (max/min) :', max_loss / min_loss)
    ...
    x = 0
    for j in range(n):
        if Ltot[j] > threshold * min_loss:
            drop_20.append(range(j * totalProcesses, (j + 1) * totalProcesses))
            x += 1

    print(f'{x} out of {n} repeats were filtered out using a {threshold_percent}% threshold')
    filt = 2 if x > 0 else 1
...
def reclustering(tempWall=0, tempHall=0, processAvg=0, exposureAvg=0, dist="cosine",gpu=False):
    ...
    for cluster_id in range(processes3D.shape[0]):
        ...
        clusters = pd.concat([clusters, cluster_vectors.T], ignore_index=True)
...
#---------------------------- alternative solution end -------------------------------------------------#
    ...
    vals = ax1.get_yticks()
    ax1.set_xticklabels(np.arange(min(t), max(t)+1, 1), rotation=30)
