...
def sigProfilerExtractor(input_type, 
    ...
    for m in mtypes:
        ...
        all_similarities_list = [] #this list is going to store the dataframes of different similarities as items
        minimum_stabilities   = []
    ...
    'f1_mean': np.mean(f1),
    'Lrec_mean': np.mean(Lrec),
    'Lce_mean': np.mean(Lce),
    'Ltot_mean': np.mean(Ltot),
    'epochs_mean': np.mean(epochs),
    'n_filter': n_filter,

    'stability_mean_refit0': stability_mean_2[0],
    'stability_min_refit0': stability_min_2[0],
    'rec_refit0': rec_2[0],
    'acc_refit0': acc_refit_2[0],
    'f1_refit0': f1_refit_2[0],
    'Lrec_refit0': Lrec_refit_2[0],
    'Lce_refit0': Lce_refit_2[0],
    'Ltot_refit0': Ltot_refit_2[0],
    'epochs_refit0': epochs_refit_2[0],

    'stability_mean_refit1': stability_mean_2[1],
    'stability_min_refit1': stability_min_2[1],
    'rec_refit1': rec_2[1],
    'acc_refit1': acc_refit_2[1],
    'f1_refit1': f1_refit_2[1],
    'Lrec_refit1': Lrec_refit_2[1],
    'Lce_refit1': Lce_refit_2[1],
    'Ltot_refit1': Ltot_refit_2[1],
    'epochs_refit1': epochs_refit_2[1]
}])
...
def test_sigProfilerExtractor(input_type,
    ...
    classnames = label.index.tolist()


    # # Check if any of the known pathway classes are in the classnames
    # known_pathways = { 'MMR', 'HR', 'BER'}
    # if known_pathways.issubset(set(classnames)):
    #     Yhat_df = pd.DataFrame(data=Y_hat, columns=allcolnames, index=['Control', 'MMR', 'HR', 'BER'])
    # else:
    #     Yhat_df = pd.DataFrame(data=Y_hat, columns=allcolnames, index=classnames)

    Yhat_df = pd.DataFrame(data=Y_hat, columns=allcolnames, index=classnames)
    
    Yhat_df.to_csv(output_path + '/Yhat_NMF.text', sep='\t')

#!TODO: multi class accuracy?
